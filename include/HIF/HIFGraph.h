...
public:
  ...
  HIFGraph(int level, int seqnum, const vector<int> &vtx,
...
private:
  ...
  }; // Index information, used for merge, applymerge, applysplit. `c` denotes
     // `child`.

  struct ChildNode {
    vector<HIFGraph *> nodevec;

    ChildNode() { nodevec.resize(2); }
    ~ChildNode() {
    ...
    HIFGraph &Child(int t) { return *nodevec[t]; }
    const HIFGraph &Child(int t) const { return *nodevec[t]; }
  ...
  vector<SkelInfo> nbinfo_;   // Information between a node and its nbnode after
                              // skeletonization.
  vector<IndexInfo>
      childreninfo_; // Index information of a node and its children.

  // Matrices data.
  // For the following matrices, the fist index is row, and the second index is
  // col.
  ...
  MatrixS
      AIIinv_; // AII^{-1} which is obtained by the LDL factorization of AII.

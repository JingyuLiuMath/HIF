...
template <typename Scalar> void HIFGraph<Scalar>::Merge() {
  ...
  // First we tell the parent what its intr, sep, nb is after we eliminate the
  // children's intr and re.
  ...
  Intersect_Sort(intr_, childrennode_.Child(0).sep_, myindex_intr1,
                 cindex_intr1);

  vector<int> &myindex_intr2 = childreninfo_[1].myindex_intr;
  vector<int> &cindex_intr2 = childreninfo_[1].cindex_intr;
  Intersect_Sort(intr_, childrennode_.Child(1).sep_, myindex_intr2,
                 cindex_intr2);

  vector<int> myindex_intr21;
  vector<int> cindex_intr21;
  Intersect_Sort(intr_, childrennode_.Child(0).nb_, myindex_intr21,
                 cindex_intr21);

  SubMatrixUpdate(AII_, myindex_intr1, myindex_intr1,
                  childrennode_.Child(0).ASS_(cindex_intr1, cindex_intr1));

  SubMatrixUpdate(AII_, myindex_intr2, myindex_intr2,
                  childrennode_.Child(1).ASS_(cindex_intr2, cindex_intr2));

  SubMatrixUpdate(AII_, myindex_intr21, myindex_intr1,
                  childrennode_.Child(0).ANS_(cindex_intr21, cindex_intr1));

  SubMatrixTransposeUpdate(AII_, myindex_intr1, myindex_intr2,
                           AII_(myindex_intr2, myindex_intr1));

  // ASI
  // A sep of the parent only belongs to the sep of one of its children.
  // If an intr and a sep belongs to the same child, we assign ASI from the
  // child's ASS. Otherwise, we assign ASI from one child's ANS or 0.
  ...
  SubMatrixUpdate(ASI_, myindex_sep1, myindex_intr1,
                  childrennode_.Child(0).ASS_(cindex_sep1, cindex_intr1));

  SubMatrixUpdate(ASI_, myindex_sep2, myindex_intr2,
                  childrennode_.Child(1).ASS_(cindex_sep2, cindex_intr2));

  SubMatrixUpdate(ASI_, myindex_sep1y, myindex_intr1,
                  childrennode_.Child(0).ANS_(cindex_sep1y, cindex_intr1));

  SubMatrixUpdate(ASI_, myindex_sep2y, myindex_intr2,
                  childrennode_.Child(1).ANS_(cindex_sep2y, cindex_intr2));
  ...
  SubMatrixUpdate(ASS_, myindex_sep1, myindex_sep1,
                  childrennode_.Child(0).ASS_(cindex_sep1, cindex_sep1));

  SubMatrixUpdate(ASS_, myindex_sep2, myindex_sep2,
                  childrennode_.Child(1).ASS_(cindex_sep2, cindex_sep2));

  SubMatrixUpdate(ASS_, myindex_sep21, myindex_sep1,
                  childrennode_.Child(0).ANS_(cindex_sep21, cindex_sep1));

  SubMatrixTransposeUpdate(ASS_, myindex_sep1, myindex_sep2,
                           ASS_(myindex_sep2, myindex_sep1));
  ...
  SubMatrixUpdate(ANS_, myindex_nb1x, myindex_sep1,
                  childrennode_.Child(0).ANS_(cindex_nb1x, cindex_sep1));

  SubMatrixUpdate(ANS_, myindex_nb2x, myindex_sep2,
                  childrennode_.Child(1).ANS_(cindex_nb2x, cindex_sep2));
...
template <typename Scalar> void HIFGraph<Scalar>::FactorClear() {

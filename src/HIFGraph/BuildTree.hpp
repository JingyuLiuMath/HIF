...
void HIFGraph<Scalar>::BuildTree(const SparseMatrixS &A, int minvtx) {
  ...
  childrennode_.nodevec[0] =
      new HIFGraph<Scalar>(level_ + 1, 2 * seqnum_, vtx1, sep1, sep2);
  childrennode_.nodevec[1] =
      new HIFGraph<Scalar>(level_ + 1, 2 * seqnum_ + 1, vtx2, sep2, sep1);
  ...
  // Set numlevels.
  numlevels_ = std::max(childrennode_.Child(0).numlevels_,
                        childrennode_.Child(1).numlevels_);
}

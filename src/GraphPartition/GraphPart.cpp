...
void MetisPart(const SparseMatrix<Scalar> &A, vector<int> &p1, vector<int> &p2,
  ...
  MetisSepPart(A(idx, idx), lidx, ridx,
               sepidx); // lidx, ridx, sepidx are sorted.
  p1.resize(lidx.size());
  p2.resize(ridx.size());
...
#define PROTOTYPE_PART(Scalar)                                                 \
  template void GraphPart(const SparseMatrix<Scalar> &A, vector<int> &p1,      \
                          vector<int> &p2, vector<int> &sep1,                  \
                          vector<int> &sep2);                                  \
  template void MetisPart(const SparseMatrix<Scalar> &A, vector<int> &p1,      \
                          vector<int> &p2, vector<int> &sep);                  \

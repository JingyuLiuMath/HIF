...
// A(rowindex, colindex) = newsubA^{T} where rowindex and colindex are sorted.
template <typename Scalar>
void SubMatrixTransposeUpdate(Matrix<Scalar> &A, const vector<int> &rowindex,
  ...
  for (int i = 0; i < rowindex.size(); i++) {
    for (int j = 0; j < colindex.size(); j++) {
      A.Set(rowindex[i], colindex[j], newsubA.Get(j, i));
    }
  }
}
...
#define PROTOTYPE(Scalar)                                                      \
  ...
  template void SubMatrixUpdate(                                               \
      Matrix<Scalar> &A, const vector<int> &rowindex,                          \
      const vector<int> &colindex, const Matrix<Scalar> &newsubA);             \
  template void SubMatrixTransposeUpdate(                                      \
      Matrix<Scalar> &A, const vector<int> &rowindex,                          \
      const vector<int> &colindex, const Matrix<Scalar> &newsubA);             \
  template void SubMatrixEqual(Matrix<Scalar> &subA, const Matrix<Scalar> &A,  \
